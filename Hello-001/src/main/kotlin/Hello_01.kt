...
fun main () {
        ...
        // 선언을 해주고 반드시 초기값을 입력해주어야한다
        var num1 = 10;
        var num2 = 20;
        var sum = num1 + num2
        /*
        ""를 문자열처럼 사용하되 $를 앞에 사용해주면
        문자열코드로 인식하여 문자열로 사용할 수 있다
        */

        // 문자열 templete 식
        // 문자열 내부에 $변수명 형식으로 작성을 하면 변수값을 연산하여 문자열로 변환 후 사용할 수 있다
        println("$num1 + $num2 = $sum")

        // ${ 연산식 } 형식으로 작성을 하면 연산식으로 연산한 후
        // 결과를 문자열로 변환한 후 사용할 수 있다
        println("${num1 * num2}")

        var str1 = "두 수 $num1 과 $num2 의 덧셈 결과 : ${num1 + num2}"
        var str2 = "두 수 ${num1}과 ${num2} 의 덧셈 결과 : ${num1 + num2}"
        println(str1)
        println(str2)

        // num1은 이미 Int형으로 선언되었기 때문에
        // 실수형 값을 저장할 수 없다
        // num1 = 3.0
       ...
       /**
         ...
         *
         * 단지 변수 type을 지정하지 않을 뿐 한번 선언된 변수에
         * 다른 type의 데이터(값)을 저장할 수는 없다
        *
        *  변수를 선언할 때 변수들의 초기값은 null 이 될 수 없다
        *
        *  kotlin에서도 변수형을 지정하여 선언할 수 있다
        *  Kotlin의 변수 type은 클래스 type이 없다.
        *  모두 primitive 방식이다.
        * 단, 모든 변수 type의 키워드는 첫 글자가 대문자이다.
        * 변수 type을 지정하지 않아도 선언할 수 있는데 굳이 type을 지정하는 이유는
        * Kotlin 문법 중 변수의 초기화를 미루기(유예하기) 라는 기능 때문이다
         */
}
